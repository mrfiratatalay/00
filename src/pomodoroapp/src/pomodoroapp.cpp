/**
 * @file pomodoroapp.cpp
 * @brief A simple program to demonstrate the usage of the pomodoro model class.
 *
...
double evaluatePostfix(const std::string &postfix) {
  ...
  while(iss >> token) {
    if(isOperator(token[0])) {
      ...
      switch(token[0]) {
        case '+':
          result = pomodoro::add(a, b);
          break;

        case '-':
          result = pomodoro::subtract(a, b);
          break;

        case '*':
          result = pomodoro::multiply(a, b);
          break;
